...
obsidian_sync() {
    ...
    local zip_file="FoxVault_${timestamp}.zip"
    ...
    if [[ "$direction" == "pull" ]]; then
        ...
        # For pull, we need to find the latest zip file
        echo "Finding most recent backup..."
        latest_backup=$(rclone lsf "$remote_base" --include "FoxVault_*.zip" | sort -r | head -n 1)
        ...
        remote_zip_file=$latest_backup
        local_zip_path="$HOME/ProtonDrive/Archives/Obsidian/$remote_zip_file"
        
        echo "Latest backup: $remote_zip_file"

        # Create local directory if it doesn't exist
        mkdir -p "$(dirname "$local_zip_path")"

        # Download the zip file
        echo "Downloading $remote_zip_file from ProtonDrive..."
        rclone copy "$remote_base/$remote_zip_file" "$(dirname "$local_zip_path")" --progress
        exit_code=$?
        ...
        echo "Extracting $remote_zip_file to $local_vault_path..."
    ...
    # Push: Zip local vault and upload to ProtonDrive
        ...
        echo "  pull: Download and extract most recent FoxVault from ProtonDrive"
